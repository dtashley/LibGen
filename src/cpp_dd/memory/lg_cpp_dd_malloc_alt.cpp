...
//
//The behavior of these wrapper functions differs from the malloc() family of functions
//in that allocation and reallocation requests for 0 bytes always return nullptr.
//--------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
#define MODULE_LG_CPP_DD_MALLOC_ALT
 ...
 */
void *LgCppCm_MallocAltMalloc(size_t in_size)
{
   void* rv = nullptr;
 ...
 */
void* LgCppCm_MallocAltCalloc(size_t in_num, size_t in_size)
{
   void* rv = nullptr;
 ...
 */
void* LgCppCm_MallocAltRealloc(void* in_memblock, size_t in_size)
{
   void* rv = nullptr;
   ...
   if (!in_memblock)
   {
      ...
      else
      {
         ...
         //be thrown.
         rv = LgCppCm_MallocAltMalloc(in_size);
      }
   }
   else
   {
 ...
 * \reentrancyandthreadsafety    This function is re-entrant and thread safe.
 */
void LgCppCm_MallocAltFree(void *in_memblock)
{
   if (!in_memblock)
   {
...
//End of lg_cpp_dd_malloc_alt.cpp.
