...
/*!
 * Determines whether an 8-bit ASCII character is largely printable without side effects (such as
 * moving a console cursor or sounding the bell.  Returns true if is largely printable, or
 * false otherwise.
 *
 * \param[in]      in_c              The character to classify.
 * \returns                          true if the character is likely to print without side effects,
 *                                   or false otherwise.
 * \reentrancyandthreadsafety        This function is re-entrant and thread safe.
 * \errorsandexceptions              No exceptions are possible.
 */
bool LgCppDd_CharfAsciiCharIsLargelyPrintable(const char in_c) noexcept
{
   if ((in_c >= ' ') && (in_c <= '~'))
      return true;
   else
